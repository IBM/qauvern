...
**Pruning invariant.** Attribution runs over the still-crediting grants only, which makes `update`'s removal of rolled-off grants provably a no-op: a grant with `end_date <= window_start(today)` contributes to no term. Under all-grants attribution a long-dead grant would absorb out-of-window usage and leave more budget in a live grant, so pruning would silently change other grants' contribution.
