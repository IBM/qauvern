...
- Replaced `enforce_usage_floor` with the more flexible option `usage_floor_relax_above_percent`. Instead of a static on/off switch, this is a percent threshold: once account usage exceeds this percent of the account's allocation budget, the invariant that an instance's allocation must stay >= its 28-day usage is relaxed. Defaults to `100`, a sentinel meaning the floor is always enforced (matching the old default). Must be in `[0, 100]`.

## [0.11.0] - 2026-08-06

### Added

- Added `enforce_usage_floor` option to config file, with default to true. Set to false to stop enforcing the invariant that an instance's allocation <= its 28-day consumption.
...
- New `update` command reconciles an existing config file with the live IBM Quantum API. It drops expired `net_grants`, adds newly discovered instances, fixes instance name drift, and removes archived/missing instances. All four actions are run by default, but can be opted out with `--no-net-grants`, `--no-add`, `--no-names`, or `--no-remove`. Use `--dry-run` to preview, and `--region` to restric to a single region.
- `analyze` has a `--format {table, csv, json}` argument (default `table`). `csv` emits one row per configured instance; `json` emits a structured payload (account info, reserve, validation errors, and per-instance rows with pre-computed allocation/limit deltas) for scripts and dashboards.
- Added `--yes` option to `optimize` and `update` to skip confirmation. This is useful for automations like cron jobs.
...
[0.10.0]: https://github.com/ibm/qauvern/releases/tag/v0.10.0
