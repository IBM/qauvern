...
from qauvern.models import Instance, NetGrant, Project
...
def test_no_limit_fields_returns_none(resolver: LimitResolver) -> None:
    ...
    assert resolver.resolve(project, instance, date(2026, 4, 27)) is None


def test_base_limit_only_returns_base(resolver: LimitResolver) -> None:
    project = make_project(project_limit_seconds=50000)
    instance = make_instance()
    assert resolver.resolve(project, instance, date(2026, 4, 27)) == 50000


def test_exhausted_returns_one(resolver: LimitResolver) -> None:
    project = make_project(project_limit_seconds=50000)
    instance = make_instance(consumed_balance=30000, target=30000)
    assert resolver.resolve(project, instance, date(2026, 4, 27)) == 1
...
def test_grant_active_on_start_date(resolver: LimitResolver) -> None:
    ...
    project = make_project(project_limit_seconds=50000, net_grants=[grant])
    result = resolver.resolve(project, make_instance(), date(2026, 5, 1))
    assert result == 90000
...
def test_grant_active_on_day_27(resolver: LimitResolver) -> None:
    ...
    project = make_project(project_limit_seconds=50000, net_grants=[grant])
    result = resolver.resolve(project, make_instance(), last_day)
    assert result == 90000
...
def test_grant_expired_on_day_28(resolver: LimitResolver) -> None:
    ...
    grant = NetGrant(start_date=start, net_grant_seconds=40000, end_date=datetime(2026, 5, 29))
    project = make_project(project_limit_seconds=50000, net_grants=[grant])
    result = resolver.resolve(project, make_instance(), day_28)
    assert result == 50000
...
def test_future_grant_returns_base_limit(resolver: LimitResolver) -> None:
    grant = NetGrant(start_date=datetime(2026, 5, 1), net_grant_seconds=40000, end_date=datetime(2026, 5, 29))
    project = make_project(project_limit_seconds=50000, net_grants=[grant])
    result = resolver.resolve(project, make_instance(), date(2026, 4, 30))
    assert result == 50000
...
def test_no_rolloff_when_grant_just_started(resolver: LimitResolver) -> None:
    ...
    result = resolver.resolve(project, instance, today)
...
def test_two_active_grants_stack(resolver: LimitResolver) -> None:
    ...
    project = make_project(project_limit_seconds=50000, net_grants=[grant1, grant2])
...
def test_active_grant_no_base_limit_returns_grant_only(resolver: LimitResolver) -> None:
    """Grant without project_limit_seconds: base treated as 0."""
    today = date(2026, 4, 27)
    grant = NetGrant(start_date=datetime(2026, 4, 27), net_grant_seconds=40000, end_date=datetime(2026, 5, 25))
    project = make_project(project_limit_seconds=None, net_grants=[grant])
    result = resolver.resolve(project, make_instance(), today)
    assert result == 40000
...
def test_grant_with_custom_end_date_active(resolver: LimitResolver) -> None:
    ...
    project = make_project(project_limit_seconds=50000, net_grants=[grant])
    result = resolver.resolve(project, make_instance(), date(2026, 5, 31))
    assert result == 90000
...
def test_grant_with_short_end_date_expired(resolver: LimitResolver) -> None:
    ...
    project = make_project(project_limit_seconds=50000, net_grants=[grant])
    result = resolver.resolve(project, make_instance(), date(2026, 5, 15))
    assert result == 50000  # end_date is exclusive, grant expired


@pytest.fixture
def rolloff_scenario() -> tuple[LimitResolver, Project, Instance]:
    ...
    return resolver, project, instance


def test_jan5_grant_start_full_grant(rolloff_scenario: tuple[LimitResolver, Project, Instance]) -> None:
    """Jan 5 (grant start): no rolloff yet, full grant active."""
    resolver, project, instance = rolloff_scenario
    assert resolver.resolve(project, instance, date(2026, 1, 5)) == 18000


def test_jan29_no_rolloff_yet(rolloff_scenario: tuple[LimitResolver, Project, Instance]) -> None:
    ...
    resolver, project, instance = rolloff_scenario
    assert resolver.resolve(project, instance, date(2026, 1, 29)) == 18000


def test_jan30_first_day_rolls_off(rolloff_scenario: tuple[LimitResolver, Project, Instance]) -> None:
    """Jan 30: today-28=Jan 2, Jan 1 exits (d < Jan 2), rolloff=1800."""
    resolver, project, instance = rolloff_scenario
    assert resolver.resolve(project, instance, date(2026, 1, 30)) == 16200  # 12000+4200


def test_jan31_two_days_rolled_off(rolloff_scenario: tuple[LimitResolver, Project, Instance]) -> None:
    """Jan 31: today-28=Jan 3, Jan 1+2 exit, rolloff=3600."""
    resolver, project, instance = rolloff_scenario
    assert resolver.resolve(project, instance, date(2026, 1, 31)) == 14400  # 12000+2400


def test_feb1_three_days_rolled_off(rolloff_scenario: tuple[LimitResolver, Project, Instance]) -> None:
    """Feb 1: today-28=Jan 4, Jan 1+2+3 exit, rolloff=5400, last active day of grant."""
    resolver, project, instance = rolloff_scenario
    assert resolver.resolve(project, instance, date(2026, 2, 1)) == 12600  # 12000+600


def test_feb2_grant_expired(rolloff_scenario: tuple[LimitResolver, Project, Instance]) -> None:
    """Feb 2: grant_end=Jan 5+28=Feb 2, today >= grant_end so grant inactive, base only."""
    resolver, project, instance = rolloff_scenario
    assert resolver.resolve(project, instance, date(2026, 2, 2)) == 12000
